...
void Entity::tick() {
    ...
    for (auto iter = components.begin(); iter != components.end(); ++iter) {
        // component tick在propertys_sync2client前，避免属性脏标记被清之后无法同步到other client
        iter->second->tick();
    }
...
void Entity::serialize_client_all(Encoder& encoder, bool need_clean_dirty) {
    for (auto iter = propertys.begin(); iter != propertys.end(); ++iter) {
...
void Entity::serialize_otherclient(Encoder& encoder) {
    for (auto iter = dirty_propertys.begin(); iter != dirty_propertys.end(); ++iter) {

        if (!iter->second->need_sync2otherclient()) {
            continue;
        }
        ...
        encoder.write_uint16(prop_str2int(iter->first));
        iter->second->serialize(encoder, false);
    }
}

void Entity::serialize_otherclient_all(Encoder& encoder) {
    for (auto iter = propertys.begin(); iter != propertys.end(); ++iter) {
        ...
        encoder.write_uint16(prop_str2int(iter->first));
        iter->second->serialize_all(encoder, false);
    }
}
...
void BaseEntityWithClient::propertys_sync2client(bool force_all) {
    Encoder encoder;
    if (force_all) {
        serialize_client_all(encoder, true);
    }
    else {
...
Entity* create_other_entity(const GString& entity_name, const GString& entity_uuid) {
    function<Entity*()> creator = get_entity_creator("Client" + entity_name);
    if (!creator) {
        ERROR_LOG("client entity type(%s) error\n", entity_name.c_str());
        return nullptr;
    ...
    DEBUG_LOG("create_other_entity %s.%s\n", entity_name.c_str(), entity_uuid.c_str());
